module Overrides
  class RegistrationsController < DeviseTokenAuth::RegistrationsController
    ...
    def create
      ...
      sign_up_params[:name]               = params[:name] if params[:name]
      sign_up_params[:remember_me]        = params[:remember_me] if params[:remember_me]
      sign_up_params[:initial_wishes]     = params[:initial_wishes] if params[:initial_wishes]
      sign_up_params[:initial_conditions] = params[:initial_conditions] if params[:initial_conditions]
      sign_up_params[:newsletter]         = params[:newsletter] if params[:newsletter]
      sign_up_params[:datenschutz]        = params[:datenschutz] if params[:datenschutz]

      user = params.permit(:name, :email, :password,
      ...
      begin
        ...
        if @resource.save
          ...
          render json: {
            success: true,
            data: {
              user: UserSerializer.new(@resource, scope: @resource)
            }
          }
        else
